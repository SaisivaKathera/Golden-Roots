...
ResultSet rs = s.executeQuery("select * from workshop where sid = "+std+"");
Statement s1 = con.createStatement();
ResultSet rs1 = s1.executeQuery("select count(*) from workshop where sid = "+std+"");
if(rs1.next()){
n = rs1.getInt(1);
}
...
String a = "accepted";
ResultSet rs2 = s2.executeQuery("select count(*) from workshop where sid = "+std+" and status = '"+a+"'");
if(rs2.next()){
p = rs2.getInt(1);
}
System.out.println(n);
...
<form method = "post" enctype="multipart/form-data" action = "workup?eve=workshop&sid=<%=std%>" name = "login_form">
<input type = "submit" value = "WORKSHOP" style="background-color:silver; font-family:georgia; color:black">
...
if(n != 0 && p != 0){
	...
	<td><a href="seminarfiledownload.jsp?semid=<%= rs.getString("semid") %>&eve=workshop">View Certificate</a></td>
	</tr>
