...
ResultSet rs = s.executeQuery("select * from onlinequiz where sid = "+std+"");
Statement s1 = con.createStatement();
ResultSet rs1 = s1.executeQuery("select count(*) from onlinequiz where sid = "+std+"");
if(rs1.next()){
n = rs1.getInt(1);
}
...
ResultSet rs2 = s2.executeQuery("select count(*) from onlinequiz where sid = "+std+" and status = '"+a+"'");
if(rs2.next()){
p = rs2.getInt(1);
}
...
<form method = "post" enctype="multipart/form-data" action = "onlineup?eve=onlinequiz&sid=<%=std%>" name = "login_form">
<input type = "submit" value = "ONLINE QUIZ" style="background-color:silver; font-family:georgia; color:black">
...
if(n != 0 && p != 0){
	...
	<td><a href="seminarfiledownload.jsp?semid=<%= rs.getString("semid")%>&eve=onlinequiz">View Certificate</a></td>
	</tr>
