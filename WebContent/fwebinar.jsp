...
ResultSet rs = s.executeQuery("select * from webinar");
Statement s1 = con.createStatement();
ResultSet rs1 = s1.executeQuery("select count(*) from webinar ");
...
ResultSet rs2 = s2.executeQuery("select count(*) from webinar where status = '"+a+"' and branch = '"+branch+"'");
if(rs2.next()){
p = rs2.getInt(1);
}
...
if(n != 0 && p != 0){
	...
	<td><a href="ffiledownload.jsp?semid=<%= rs.getString("semid") %>&eve=webinar">View Certificate</a></td>
	
	<td>
	<form method = "post" action = "fupdateseminar.jsp?semid=<%=semid%>&status=accepted&eve=webinar">
	<input type = "submit" value = "Accept">
	</form>
	<form method = "post" action = "fupdateseminar.jsp?semid=<%=semid%>&status=declined&eve=webinar">
