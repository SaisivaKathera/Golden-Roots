...
@MultipartConfig(maxFileSize = 16177215)
 @WebServlet("/hackup")
public class hackathonupload extends HttpServlet {
...
String cont = request.getParameter("contribution");
	   ...
	   //     s.executeUpdate("insert into seminar(sid,category,name,fromd,tod,orgby,loc,branch,pdf) values("+sid+",'"+category+"','"+aname+"','"+fdate+"','"+tdate+"','"+ob+"','"+loc+"','"+b+"',"+bytes+")");
	        PreparedStatement statement = null;
	         String sql = " insert into hackathon(sid,domain,fromd,tod,orgby,nmt,contribution,branch,pdf) values(?,?,?,?,?,?,?,?,?)  ";
	         statement = con.prepareStatement(sql);
	          ...
	          statement.setString(5, ob);
	          statement.setString(6, nmt);
	          statement.setString(7, cont);
