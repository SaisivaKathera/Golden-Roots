...
@MultipartConfig(maxFileSize = 16177215)
 @WebServlet("/challengeup")
public class globalchallengeupload extends HttpServlet {
...
String other = request.getParameter("other");
	   ...
	   //     s.executeUpdate("insert into seminar(sid,category,name,fromd,tod,orgby,loc,branch,pdf) values("+sid+",'"+category+"','"+aname+"','"+fdate+"','"+tdate+"','"+ob+"','"+loc+"','"+b+"',"+bytes+")");
	        PreparedStatement statement = null;
	         String sql = " insert into globalchallenge(sid,category,name,fromd,tod,certify,branch,pdf,other) values(?,?,?,?,?,?,?,?,?)  ";
